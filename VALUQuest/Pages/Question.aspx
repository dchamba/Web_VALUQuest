...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="form-check" style="margin-top: 30px;">
                                <input class="form-check-input" type="checkbox" id="chkIsBMI">
                                <label class="form-check-label" for="chkIsBMI">Mostra domanda in base a BMI</label>
                            </div>
                        </div>

                    </div>

                    <!-- Row for BMI min and max values, initially hidden -->
                    <div id="bmiValuesRow" class="row mb-3" style="display: none;">
                        <div class="col-md-3">
                            <label class="form-label">Valore BMI >=</label>
                            <input type="text" class="form-control form-control-sm" id="isBMI_minValue">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Valore BMI <=</label>
                            <input type="text" class="form-control form-control-sm" id="isBMI_maxValue">
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#chkIsBMI').change(function () {
                if ($(this).is(':checked')) {
                    $('#bmiValuesRow').show(); // Show the row with textboxes
                } else {
                    $('#bmiValuesRow').hide(); // Hide the row with textboxes
                    $('#isBMI_minValue').val(''); // Clear the values
                    $('#isBMI_maxValue').val('');
                }
            });
 ...
 function toggleCategoryDetails(blockName, categoryName, buttonElement, searchTerm = '') {
    ...
    if (isExpanded) {
        ...
        // Adding the three new columns: Is BMI, BMI Min Value, and BMI Max Value
        headerRow.append('<th>Is BMI<i class="mdi mdi-arrow-up"></i></th>');
        headerRow.append('<th>BMI Min Value<i class="mdi mdi-arrow-up"></i></th>');
        headerRow.append('<th>BMI Max Value<i class="mdi mdi-arrow-up"></i></th>');
        ...
        filteredData.forEach(function (item) {
            ...
            // Adding the new data columns: Is BMI, BMI Min Value, and BMI Max Value
            row.append($('<td>').text(item.isBMI == 1 ? 'Yes' : '')); // Display 'Yes' or 'No' for isBMI
            row.append($('<td>').text(item.bmiminValue || '')); // Display BMI Min Value
            row.append($('<td>').text(item.bmimaxValue || '')); // Display BMI Max Value
        ...
        innerTableRow.html($('<td>').attr('colspan', '12').append(innerTable)); // Adjust colspan to exclude removed column
        innerTableRow.show();
        ...
        function saveData() {
            ...
            if (isBMI) {
                if (!bmiMinValue || !bmiMaxValue) {
                    showAlert('Errore inserimento dati', 'danger'); // Show error message if either value is empty
                    return;
                }
            }
            ...
            setTimeout(function () {
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({
                        ...
                        maxValue: maxValue,
                        unit: unit,
                        isBMI: isBMI,          // Pass the isBMI checkbox status
                        bmiMinValue: bmiMinValue, // Always pass BMI min value
                        bmiMaxValue: bmiMaxValue  // Always pass BMI max value
                    }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
        ...
        function editData(item) {
            ...
            // Handle MCQ question type
                ...
                var questionId = item.questionId;
                $.ajax({
                    url: '/Pages/Question.aspx/GetOptionsData',
                    method: 'POST',
            ...
            // Set the Is BMI, BMI Min Value, and BMI Max Value fields
            if (item.isBMI == 1) {
                $('#chkIsBMI').prop('checked', true); // Check the "Is BMI" checkbox
                $('#bmiValuesRow').show(); // Show the BMI min/max value row
                document.getElementById('isBMI_minValue').value = item.bmiminValue; // Set BMI Min Value
                document.getElementById('isBMI_maxValue').value = item.bmimaxValue; // Set BMI Max Value
            } else {
                $('#chkIsBMI').prop('checked', false); // Uncheck the "Is BMI" checkbox
                $('#bmiValuesRow').hide(); // Hide the BMI min/max value row
                document.getElementById('isBMI_minValue').value = ''; // Clear BMI Min Value
                document.getElementById('isBMI_maxValue').value = ''; // Clear BMI Max Value
            }
            ...
            setTimeout(function () {
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({
                        ...
                        maxValue: maxValue,
                        unit: unit,
                        isBMI: isBMI,           // Add the isBMI value
                        bmiMinValue: bmiMinValue, // Add the bmiMinValue
                        bmiMaxValue: bmiMaxValue  // Add the bmiMaxValue
                    }),
        ...
        function clearData() {
            ...
            $('#tblMCQSOptions tbody').empty();
            // Reset the new BMI-related fields
            $('#chkIsBMI').prop('checked', false); // Uncheck the 'Is BMI' checkbox
            $('#isBMI_minValue').val('');         // Clear the BMI Min Value field
            $('#isBMI_maxValue').val('');         // Clear the BMI Max Value field

            // Hide the BMI value fields initially when clearing data

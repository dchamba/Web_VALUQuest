    ...
    <script>
        $(document).ready(function () {
            ...
            $('#searchButton').click(function () {
               ...
               if ($('#ddlBlock option').length === 0) {
                ...
                // If subcategory dropdown has no items, set subcategoryId to -1
                ...
                loadQuestions(blockId, categoryId, subcategoryId);
            });
        ...
        function changeSubCategory(catId, subcatId) {
            ...
            setTimeout(function () {
                $.ajax({
                    type: "POST",
                    url: "/Pages/FixedQuestion.aspx/GetSubCategories",
        ...
        function saveData() {
            ...
            $('#tblQuestionsData tbody tr').each(function () {
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({ questionId: questionId, isChecked: isChecked }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        // Handle success
        ...
        function loadQuestions(blockId, categoryId, subcategoryId) {

            $.ajax({
                type: "POST",
                url: "/Pages/FixedQuestion.aspx/GetQuestions",
                data: JSON.stringify({ blockId: blockId, categoryId: categoryId, subcategoryId: subcategoryId }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    var data = JSON.parse(response.d);
                    ...
                    $.each(data, function (index, item) {
                        var newRow = '<tr>' +
                            ...
                            '<td>' + item.quesType + '</td>' +
                            '<td>' +
                            '<input type="checkbox" class="form-check-input" ' + (item.isFixed === 1 ? 'checked' : '') + '>' +
                            '</td>' +
                            '</tr>';
